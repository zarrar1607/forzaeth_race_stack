...
# Global variables
drive = '/vesc/high_level/ackermann_cmd_mux/input/nav_1'  # Topic for drive commands
...
bag_name = 'Dataset/ForzaETH2.bag' # path and name of the bag where the data needs to be stored
...
# Function to handle key press and toggle recording
def on_press(key):
    global pressed
    global bag
    ...
    try:
        ...
        if key.char == 'r' and not pressed:
            pressed = True
            bag = rosbag.Bag(bag_name, 'w')  # Create a new bag for recording
            print('Recording Started')
        # Stop recording on 's' key press
        elif key.char == 's' and pressed:
            pressed = False
            bag.close()  # Close the bag when recording stops
            print('Recording Stopped')
