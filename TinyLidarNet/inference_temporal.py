...
from collections import deque  # For maintaining a rolling buffer
...
model_name = './Models/TLN_temporal_info_noquantized.tflite'
...
def callback(l):
    ...
    lidar_queue.append(ldata)  # Add new scan to the queue
...
def dnn_output():
    ...
    # Create a copy of lidar_queue to avoid mutation during iteration
    lidar_queue_copy = list(lidar_queue)

    # Ensure each scan has 540 points
    lidar_resized = [np.resize(scan, (len(lidar_queue_copy[0][0]), 1)) for scan in lidar_queue_copy]
    # Stack the scans along the temporal axis
    l_data = np.stack(lidar_resized, axis=0)  # Shape: (5, 540, 1)
    l_data = np.expand_dims(l_data, axis=0)  # Add batch dimension: (1, 5, 540, 1)

    interpreter.set_tensor(input_index, l_data)
...
while not rospy.is_shutdown():
    ...
    speed = linear_map(speed, 0, 1, -0., 8.0)
